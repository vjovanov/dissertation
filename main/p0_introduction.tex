...
In this chapter we provide a brief introduction to the Scala Programming Language~\cite{odersky_scala_2004}~(\S \ref{sec:scala}) and to embedded domain-specific languages in Scala~(\S \ref{sec:embedded-domain-specific-languages}). %and to partial evaluation and staging~(\S \ref{sec:introdution-partial-evaluation}).
...
\paragraph{Object oriented features of Scala.} Scala's types are organized in a
 \emph{type hierarchy}. At the \emph{top} of the hierarchy is the \code{Any} type
...
% Components, classes, traits paths.
...
in Scala are internally represented as classes with an \code{apply} method that defines
