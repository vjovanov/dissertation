...
At the \emph{top} of the hierarchy is type \code{Any} with its two subtypes \code{AnyVal} and \code{AnyRef}. Type \code{AnyVal} is the supertype of all primitive types (e.g., \code{Int}) while \code{AnyRef} is the supertype
...
At the bottom of the hierarchy is the type \code{Nothing} which has no corresponding
 instances. Type \code{Null} is a subtype of all reference types and has a single instance \code{null}.

\paragraph{The Scala components.}
...
as terms with a special syntactic construct (e.g., \code{(x: Int)=> x + 1}). The functions
in Scala are internally represented as classes with an \code{apply} method that holds
the body of the function. For each cardinality of the function there is a corresponding
class. For example, function \code{(x: Int)=> x + 1} is represented with:\begin{lstparagraph}
class anonymous$\$$uid extends Function2[Int, Int] {
  def apply(x: Int) = x + 1
}
\end{lstparagraph}

\todo{Function currying.}
...
The Scala function evaluation, by default, first executes the method arguments and then
 ...
 the type of the function parameter. A by-name parameter of type \code{Int} is written as
 \code{p: => Int}.


% Pattern matching ~> unapply pattern
Scala supports \emph{pattern matching} over terms.  In Scala the \code{case classes} are
...
\emph{extractor} for deconstruction. Extractor is a method named \code{unapply} that
...
can be implicit, and they are declared so by writing the keyword \code{implicit} in the beginning
of the parameter list. Inside the method definition implicit parameters are further treated as implicit
definitions. For example, a code generation method can accept an implicit \code{PrintStream} by declaring it implicit:\begin{lstparagraph}
...
\smartparagraph{Type classes.} Type classes are in Scala~\cite{oliveira_type_2010} introduced
 ...
 is achieved by defining a trait for that type class, e.g., \code{Numeric[T]}. Then a type class
...
\paragraph{Implicit conversions.} Scala allows user-defined implicit conversions, besides the standard implicit conversions
 ...
 that have a single non-implicit parameter list. An implicit conversion for an abstract type \code{Rep[Int]}
...
tries to find the implicit conversion method that would ``fix'' the program and have correct types. The


\smartparagraph{Extension methods.}
% Extension methods
...
thesis. \todo{fix this} We briefly explain the core concepts of Lightweight Modular Staging
