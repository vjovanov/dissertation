...
Infrastructure of our society is controlled by billions of lines of code executed in data-centers,
 ...
 and C\#. Advancements in our society's infrastructure are depend on evolution of the code that controls it and
 evolution of code is influenced by \emph{programmer productivity}.
...
Modern general-purpose programming languages make programmers productive by
 providing language constructs with \emph{high levels of abstraction}. Abstract constructs foster
 writing concise programs that are easy to comprehend. Unfortunately, abstraction comes with a cost: each layer of
 abstraction must be executed on a \emph{target platform}. This execution takes time and energy, i.e.,
 computations with high-level of abstraction take more time and energy.

% The performance is important for our environment.
Inefficient and long running programs slow down decision making and use excess energy
...
Writing efficient programs in general-purpose programming languages, however, leads to low productivity.
...
\paragraph{Why compilers can not optimize high-level programs?} For compilers of
 general-purpose programming languages it is hard to remove the abstraction overheads
 and at the same time target heterogeneous platforms. The two main reasons general
 purpose compilers can not achieve these goals are:\begin{itemize}

 \item General purpose compilers reason about general computations and can not reason
   about \emph{specific domains} such as linear algebra and relational algebra. This reduces
   the number of possible optimizations they can apply to the programs.

 \item During compilation compilers are faced with a overwhelming number of choices
...
Domain-specific languages (DSLs) provide a \emph{restricted} interface that allows
 ...
 and to guide the optimizer in exploring that space. The restricted interface and domain knowledge can
 further be used to execute on heterogeneous targets~\cite{chafi_language_2010}.
...
A successful example of a DSL is the Standard Query Language (SQL) with millions of active
 ...
 the knowledge about relational algebra to optimize data queries. SQL, as such,
 provides the base for almost all enterprise applications in the world.
...
infrastructure: \emph{i)} IDE support, \emph{ii)} tools (e.g., debuggers and code analysis), \emph{iii)} compilation pipeline (e.g., parser, type-checker, optimizations, and code generation), and \emph{iv)} standard library. Since general-purpose languages are designed for general
 purpose code and the \todo{standard library growth of embedded DSLs is well supported}. Many
...
In this section we will show a brief comparison of programmability and performance
 ...
 user studies which we did not perform, but we rather build on informal collective experience.
...
For the DSL authors developing external DSLs is a different story. Although, the development
 ...
 external DSLs as \emph{hard to develop}. % cite
...
Finally, external DSLs have \emph{high performance}. The language and the compiler can be
 ...
 knowledge to define and explore the entire search space in order to find optimal
 programs.
...
\paragraph{Comparison summary.} We summarize different DSL kinds in \tabref{tbl:comparison}.
...
For languages whose domain allows transformations based on the domain knowledge external
 ...
 the domain knowledge from the programs and use it for optimizations. Some DSL authors
...
%%
 ...
 In the introductory part~(\S \ref{sec:introdution-partial-evaluation}) we evaluate existing approaches to predictably remove abstraction overheads in libraries. Then we show that none of
  ...
  approaches like \code{static} terms in D are not powerful enough. Approaches like staging (both quotation and type based) require code duplication on the part of the DSL author which hampers development of shallow DSLs.
