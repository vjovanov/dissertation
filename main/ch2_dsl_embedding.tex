...
In the deep embedding the corresponding type error contains \code{Rep} types and the \code{this} qualifier:\vspace{3pt}
\begin{listingtiny}
  found   : Int(1)
...
{\bf \emph{Language virtualization}} allows to redefine intrinsic constructs of the
...
et al. in the modified Scala compiler named Scala-Virtualized~\cite{rompf_scala-virtualized:_2009}.
 ...
 variable binding. Furthermore, Scala is designed such that the types \code{Any} and
 ...
 unmodified Scala we must virtualize all methods on \code{Any} and \code{AnyRef}.

\begin{figure}
  \footnotesize
    \infyy{}
    {\tctx{\Gamma}{t: T_2}}
    {x: T_1 \Rightarrow t}{\mathtt{lam}[T_1,T_2](x: T_1 \Rightarrow \trone{t})}\vspace{1em}

    \infyy{}
    {\tctx{\Gamma}{t_1.f: [T_1](\Func{T_2}{T_3})}}
    {t_1.f[T_1](t_2)}{\mathtt{app}[T_2,T_3](\ttrone{t_1}.f[T_1],\ttrone{t_2})}\vspace{1em}

  \infyyax{}
  {\mathbf{def}\;f[T_1](x:T_2):T_3 {=} t}{\mathbf{def}\;f[T_1]:(T_2 {\Rightarrow} T_3){=}\trone{x:T_2 {\Rightarrow} t}}\vspace{1em}

  \infyyax{}
  {\mathbf{val}\;x:T = t}{\mathbf{val}\;x:T = \mathtt{valDef}[T](t)}\vspace{1em}

  \infyy{}
  {\tctx{\Gamma}{\mathbf{if}(t_1) \;t_2\; \mathbf{else}\;t_3: T}}
  {\mathbf{if}(t_1) \;t_2\; \mathbf{else}\;t_3}{\text{\texttt{ifThenElse}}[T](\ttrone{t_1}, \ttrone{t_2}, \ttrone{t_3})}

\caption{Subset of rules for language virtualization.}
\label{fig:virt}
\end{figure}

 \figref{fig:translation-example} illustrates this process and \figref{fig:virt}
...
definitions\footnote{In Scala, the \code{def} keyword is used to define
(possibly recursive) methods. This is similar to the \code{let} and \code{letrec}
...
\item Custom messages for unsupported host language constructs:
\begin{listingtiny}
