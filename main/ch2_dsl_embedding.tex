...
DSL intrinsification maps directly embedded versions
 ...
 need to be converted are: \emph{i)} DSL operations, \emph{ii)} DSL types,
 \emph{iii)} constant literals, and \emph{iv)} captured variables in the direct program.
...
\label{sec:constants-and-free-variables}
 ...
 \emph{Constants} can be intrinsified in the deep embedding in multiple ways.
 They can be converted to a method call for each constant (e.g., $\trone{\mathtt{1}} = \mathtt{\_\_1}$),
  type (e.g., $\trone{\mathtt{1}} = \,$\code{liftInt(1)}), or with a unified polymorphic function (e.g., $\trone{\mathtt{1}} = \,$\code{lift[Int](1)}). In the example, we use the polymorphic function approach for the constants.

\emph{Free variables} are external variables captured by a direct \edsl{}
...
The \emph{operation translation} maps directly embedded versions of the
  ...
  The \code{opMap} function in Scala injects a direct \edsl body into the context of the deep embedding.
  ...
  \paragraph{Operation \code{rebind}.} Operations in Scala are resolved through a \emph{path}. For example, an access to the method
    \code{println} of the Scala object \code{Predef}\begin{lstparagraph}
      scala.Predef.println
    \end{lstparagraph}
    has a path \code{scala.Predef}\footnote{Scala packages have a hidden prefix \code{_root_} that we always omit for simplicity.}.

    \todo{exact rules}

  \paragraph{Operation \code{inject}.} The injection of the deep DSL definitions into the scope can be achieved in multiple ways.
    For \yy we chose to inject the body of our DSLs into the refinement of the Scala
    component that contains all deep DSL definitions.

    \todo{continue}   For deep embeddings based on LMS, or polymorphic embeddings~\cite{hofer_polymorphic_2008}
  in general, \code{opMap} simply injects operations into the scope of the deep \edsl (i.e.,\ by adding the prefix
  \code{this}).


    % Prefixes that contain `this` are disregarded as they represent values of the
    % current class that can not be a part of the DSL definition.
  ...
  % implementation of \yy, the \code{opMap} function is fixed to simply inject the
  % \code{this} prefix.
