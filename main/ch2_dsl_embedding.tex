...
\begin{itemize}
  ...
  \item It improves error reporting~(\S \ref{sec:error-reporting}) in the direct embedding by:
   ...
   compile-time~(\S \ref{sec:compile-time-error-reporting}) and \emph{ii)}
...
deep embedding of DSLs can impose long compilation times~(\S \ref{sec:long-compilation-times}), and how deep embedding
becomes more complicated in the deep embedding~(\S \ref{sec:scala-specific-problems}).
...
In DSLs that use code generation, method signatures must be
 ...
 like \code{TypeTag} and \code{SourceContext}. A method \code{map} from the introductory example~(\S \ref{sec:introduction-yy}) with source and type information has two additional implicit parameters:\begin{lstparagraph}
...
While describing all the abstraction leaks in the deep embedding~(\S \ref{sec:motivation})) of the
