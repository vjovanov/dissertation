...
type translations: \emph{i)} identity translation (\sct{sct:shallow-embedding}), \emph{ii)}, \emph{iii)}, and \emph{iv)}. Finally,
we discuss how the deep embedding is compiled in \sct{sct:compilation-interface}


% In the following sections we show this interface for each type translation:
% For brevity we use the type R[_] instead of Rep[T] to represent the deep embedding.
...
trait PolymorphicControlScala extends PolymorphicBase {
  ...
  def $\$$try[T](body: => R[T], catches: Throwable => T, finalizer: => T): T
  def $\$$throw(t: Throwable): Nothing
}
