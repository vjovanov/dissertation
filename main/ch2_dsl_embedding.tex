...
in \sct{sct:langauge-virtualization} and DSL intrinsification
in \sct{sct:dsl-intrinsification}. The whole translation is explained on a concrete example
in \sct{sct:translation-example}. Finally we argue for correctness of the translation
in \sct{sct:correctness}.
...
\emph{ii)} constants and free variables, \emph{iii)} methods defined by the direct embedding. Further, for
 ...
 how to define the deep embedding interface~(\sct{sct:relaxed-interface}) for relevant type translations:
   \emph{i)} the identity translation~(\sct{sct:identity-embedding}),
   \emph{ii)} the polymorphic embedding~(\sct{sct:polymorphic-embedding}),
   \emph{iii)} the polymorphic embedding with eager inlining~(\sct{sct:polymorphic-embedding-with-eager-inlining}), and
   \emph{iv)} the embedding with custom types~(\sct{sct:embedding-with-custom-types}). For conciseness,
...
in \sct{sct:translation}. Method has an additional parameter for the run-time type
...
as we will see in \sct{sct:embedding-with-custom-types}.
...
In this section we show how to define a DSL with \yy, given that we have both the direct and deep embeddings.
First we describe the basic interface of \yy. Then how to apply the
translation~(\sct{sct:compilation-interface}). Finally, we show how the DSL user
uses \yy based DSLs.

\subsection{\yy Interface}
\label{sct:yy-interface}

\subsection{Defining a Translation for a DSL}
\label{sct:defining-a-translation}

\subsection{Using a \yy DSL}
\label{sct:using-a-dsl}
