...
should have an interface that corresponds to methods emitted by phases: \emph{i)} language virtualization,
\emph{ii)} constants and free variables, \emph{iii)} methods defined by the direct embedding. Further, for
 each type translation, signature of these method is different. In this section we describe
 how to define the deep embedding interface~(\sct{sec:relaxed-interface}) for relevant type translations:
   ...
   \emph{iv)} the embedding with custom types~(\sct{sec:embedding-with-custom-types}). For conciseness,
   in all embeddings we omit the interface of pattern matching and virtualized universal methods.\todo{code-pointer}
...
does not conform to the typical definition of an interface. In most of the translation rules
the method name, number of type arguments, and number of explicit type arguments are defined.

The number of implicit arguments and the return type, however, are left open to
 the DSL author. These arguments are added by the type-checker after scope injection. For example, if we define the
\code{$\$$ifThenElse} function as:\begin{lstparagraph}
def ifThenElse[T](cond: Boolean, thn: => T, els: => T)(
  implicit tpe: TypeTag[T]): T = \\...
\end{lstparagraph}
the Scala type-checker will invisibly provide run-time type information
to the method.
...
positions in the direct embedding source code, \emph{iii)} changing the method return type
based on the argument types, and \emph{iv)} allowing multiple methods with the
same erased signature\footnote{On the JVM, due to type erasure, methods with different
 types are sometimes erased to the same signature. This can be avoided by augmenting
 methods with an additional implicit parameters that modifies the method signature.}.

% TODO Expansion point: maybe expand and explain all of these methods in the appendix.
...
types are unmodified and, thus, can not abstract over the DSL intermediate representation.
This translations is still interesting as it can be used for instrumenting language
constructs of Scala, and for restricting certain language features in the direct embedding. The
basic interface that contains all the language features, without additional implicit parameters,
is defined in~\figref{fig:interface-identity-dsl}.

% Hole and lift interface.
In this interface method \code{hole} has a slightly different representation than what is presented
in \sct{sec:translation}. Method has an additional parameter for the run-time type
information~(\code{tpe: TypeTag[T]}). This parameter is used for explicitly passing
the type information to the method by \yy. This allows the DSL author to have a signature
of \code{hole} with multiple type parameters. This is necessary in for the \todo{embedding with custom types}
as we will see in \sct{sec:embedding-with-custom-types}.
...
With this embedding the order of execution in all control flow constructs of Scala is preserved with
 by-name parameters. For example, in \code{$\$$ifThenElse} both the \code{thn} and the \code{els} branch
...
With this type of the embedding the DSL author can instrument Scala language features
 at will. For example, collecting profiles of \code{if} conditions can be simply added by overriding the
\code{$\$$ifThenElse} method:\begin{lstparagraph}
def ifThenElse[T](cond: Boolean, thn: => T, els: => T)(implicit sc: SourceContext): T = {
...
\end{lstparagraph}
Achieving this with a facility like macros would require the DSL author to know the
reflection API and to write a transformers.

With this type of embedding the supported features that are not of interest in the deep embedding
 can be removed with an interesting trick. If we implement each method in the embedding as a macro
 that inlines the method to its original language construct all abstraction overhead
...
Library functions in the deep embedding are added by the mechanism of extension methods. However,
compared to the deep embeddings in Scala (e.g., LMS) the DSL author must take care of:
\emph{i)} how are objects treated in the direct embedding and \emph{ii)} how are direct embedding operations defined.

% Lifting objects
\paragraph{Objects in the deep embedding.}

\paragraph{Operations in the direct embedding.}
...
\label{fig:polymorphic-embedding-weat-eager-inlining}
\end{figure}
...
In this section we show how to define a DSL with \yy, given that we have
 both the direct and deep embeddings. First we describe the basic interface
 of \yy. Then how to apply the translation~(\sct{sec:defining-a-translation}).
 Finally, we show how the DSL user uses \yy based DSLs.
...
% TODO \subsection{Compilation Times}
