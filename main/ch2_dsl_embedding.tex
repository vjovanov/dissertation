...
% TODO
  % Function/method virtualization
  % test one more time for completeness
  % Talk about the infix notation.
  % Discuss the `def f` struggle.
  % Discuss All vs final members?
  % Discuss dynamic dispatch with infix methods.
  % Interaction with the pattern matcher.
  % Discuss partial functions and pattern matching.
  % Unique strings for avoiding collisions.
  % Implement the try/catch construct
  % Reynolds
  % Link better to the example and explain what is going on.
  % Discuss control constructs.

 \tool follows the ideas of Carette et al.~\cite{carette_finally_2009}
 and Scala-Virtualized but uses macros of unmodified Scala
 to virtualize Scala intrinsics which are required to write direct DSL programs. This design
 decision makes adoption of \tool easier. Practice has shown that DSL authors are reluctant to
 use a modified compiler and that for the wide adoption of embedded DSLs it is important
 to provide a solution based on unmodified Scala.
...
% Discuss control structure.
 \paragraph{Control constructs.} We translate all Scala control constructs (e.g.,\code{if} and \code{try} to method calls.
  ...
  the semantics of all constructs.

% Discuss variables.
 \paragraph{Value bindings.} \todo{}
 ...
 \tool visualizes all methods on types \code{Any} and \code{AnyRef}. Method calls on objects
  are translated into the representation where the this pointer is passed as the first argument.


  % The adequate method is chosen by overloading resolution.

  % The virtual dispatch must be handled
...
\begin{multicols}{2}
    \infyy{}
      {\tctx{\Gamma}{x: T} \quad \;\; x \; \text{is\;a\;variable}}
      {x}{\mathtt{readVar}[T](\ttrone{x})}
