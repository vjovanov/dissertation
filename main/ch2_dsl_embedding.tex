...
\label{sec:constants-and-free-variables}
 ...
 They can be converted to a method call for each constant (e.g., $\trone{\mathtt{1}} = \mathtt{\_\_1}$),
  ...
  In \yy we use the polymorphic function approach for the constants and translate constants
...
This phase of the translation simply translates paths of all operations so they bind
...
\paragraph{Operation \code{inject}.} The injection of the deep DSL definitions
 into the scope can be achieved in multiple ways.

For \yy and polymorphic embeddings~\cite{hofer_polymorphic_2008}
...
identity. If the deep embedding has corresponding signatures all operations
will rebind by re-type-checking the program in the new context, i.e., to the
operations of the deep embedding.
...
Further injection in the DSL scope allows the deep embedding to introduce additional implicit arguments to
