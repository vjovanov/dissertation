...
In \sct{sct:introduction} we introduced domain-specific languages and how they can
...
Once we ``broke the ice'' by using reflection it becomes simpler to further
 ...
 reduces long compilation times of the deep embedding by directly calling reification logic,
 and reduces run-time overheads by compiling the deep programs at host language compile-time.
...
\begin{itemize}
  ...
  \item It allows changing different deep embedding back-ends with simple configuration
   ...
   in~\sct{sec:deep-embedding-implementations}.

  \item It improves error reporting~(\sct{sec:error-reporting}) in the direct embedding by:
   \emph{i)} allowing domain-specific error reporting at host language
   compile-time~(\sct{sec:compile-time-error-reporting}) and \emph{ii)}
   restricting host language features in the direct \edsl{} based on the
   supported features of the deep \edsl{}~(\sct{sec:restricting}).

  \item It reduces the run-time overheads of the deep embedding~(\sct{sec:reducing-runtime-overheads}). The deep embeddings
  ...
  \item It reduces large compilation times introduced by the deep embedding. The translation
   ...
   in~\sct{sec:minimizing-compile-times}.
...
We evaluate \yy as a tool for generating a user-friendly frontend for the Slick DSL~\cite{slick}. The evaluation shows
that developing a frontend for existing DSLs requires little effort and that
developing an API with \yy takes far less time than the deep embedding. The complete evaluation is presented in \sct{sec:ch2-evaluation}.
...
To motivate this idea we consider the direct embedding and the deep embedding of a simple \edsl for manipulating vectors. Here, we use Scala to show the problems with the deep embedding that apply to other statically typed programming languages (e.g., Haskell and OCaml). These languages achieve the embedding in different ways~\cite{svenningsson_combining_2012,awesome,carette_finally_2009,guerrero_implementing_2004}, but this is always reflected in the type signatures. In the context of Scala, there are additional problems with type inference and implicit conversions that we discuss in \sct{sec:scala-specific-problems}.
...
The programs in the deep embedding construct the IR instead of the values in the embedded language and this inevitably leaks to the users. In this section we discuss how DSL interfaces are convoluted in the deep embedding~(\sct{sec:convoluted-interfaces}), how debugging is made difficult~(\sct{sec:difficult-debugging}), how type errors can
become incomprehensible~(\sct{sec:incomprehensible-type-errors}), how it is not possible
to restrict certain host language constructs~(\sct{sec:unrestricted-language}), how
domain-specific error reporting can only be achieved at run time~(\sct{sec:domain-specific-error-reporting-runtime}),
how run-time compilation at host-language runtime creates execution overheads~(\sct{sec:runtime-overheads}), how
deep embedding of DSLs can impose long compilation times~(\sct{sec:long-compilation-times}), and how deep embedding
becomes more complicated in the deep embedding~(\sct{sec:scala-specific-problems}).
...
In DSLs that use code generation, method signatures must be
 ...
 like \code{TypeTag} and \code{SourceContext}. A method \code{map} from the introductory example~(\sct{sec:introduction-yy}) with source and type information has two additional implicit parameters:\begin{lstparagraph}
...
Domain-specific languages often provide additional program analysis and verification
 ...
 Ideally, error reporting with the domain-specific analysis should be performed at
 compile-time in order to avoid run-time errors in production and to provide faster reaction
 time.
...
Deep EDSLs do not support compile-time error reporting due to their execution at host language run time.
 This requires users to execute the DSL body in order to perform domain-specific error reporting. This
 can lead to accidental errors in production code, requires more time find the errors as the error reporting
 requires running tests, and errors are can not be nicely integrated into the host language error
...
The deeply embedded DSLs are compiled at run time when the code is executed. This compilation
 ...
 executions depend on the implementation of the deep embedding. Some deep embeddings
 recompile their programs every time~\cite{rompf2012lightweight,slick} which can lead to significant
 execution overheads~\cite{taha_multi-stage_1997}. Others support guards for re-compilation which lower
...
Scala specific features like \emph{weak least upper bounds}~\cite{odersky_scala_2004} and \emph{type erasure} lead to further abstraction leaks in the deep embedding. In this section we discuss those and show
how they affect the DSL user.
...
\paragraph{Weak least upper bounds.} The Scala language introduces \emph{weak type conformance} where
 ...
 language specification~\cite{odersky_scala_2004} type $T$ weakly conforms to $U$~($T <:_\omega U$) if $T$ is a subtype of $U$ ($T <: U$) or $T$ precedes $U$ in the following ordering:\begin{lstparagraph}
 Byte $<:_\omega$ Short $<:_\omega$ Int
 Char $<:_\omega$ Int
 Int $<:_\omega$ Long $<:_\omega$ Float $<:_\omega$ Double
 \end{lstparagraph}

The weak least upper bounds are computed with respect to weak conformance in: \emph{i)} inferring the return type of conditional statements (e.g., \code{if} and pattern matching), and \emph{ii)} type inference of type arguments. For example, term \code{if (cond) 1 else 1.0d} has a type \code{Double}. Weak least upper bounds are computed before
the numeric conversions so the \code{then} branch of the previous term is widened to \code{1.0d}.

In case of the deep embedding there are no weak least upper bounds are applied
 as term types are not primitive. This leads to inconsistent behavior with the host
 language. For example, type checking \begin{lstparagraph}
val res = if(pos) 1 else -1.0
res * 42
\end{lstparagraph}leads to the following type error:\begin{lstparagraph}
error: value * is not a member of Prog.this.Rep[AnyVal]
res * 42
    ^
\end{lstparagraph} This behavior in the deep embedding can not be improved with implicit conversions
 as the programs are type correct and implicit conversions are
 triggered only for incorrect programs.
...
\paragraph{Type erasure.} Scala introduces \emph{erasure}~\cite{odersky_scala_2004} of abstract types to
 ...
 argument types erase to the same signature. Two methods having the same signature is illegal
 and leads to compilation errors.


 In the deep embedding all types are abstract and the methods that could be defined regularly
  become illegal. Defining methods \begin{lstparagraph}
 def from(json: Rep[String]): Rep[Vector[Double]]
 def from(bytes: Rep[Array[Byte]]): Rep[Vector[Double]]
 \end{lstparagraph} yields the following error
 which would be otherwise legal code in the host language.

 The DSL users are left with two options: \emph{i)} to rename one of the methods and diverge from the original design (e.g., rename \code{from} to \code{fromBytes}, or \emph{ii)} to introduce additional implicit parameters which will disambiguate the two methods after erasure \begin{lstparagraph}
 def from(json: Rep[String]): Rep[Vector[Double]]
