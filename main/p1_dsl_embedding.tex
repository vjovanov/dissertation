...
Heeren et.al~\cite{heeren2003scripting,hage2007heuristics} introduce extensible
 type error reporting that can be used to improve error messages. The deep embedding
 can use their tool to improve incomprehensible error messages in the deep embedding.

Svenningsson and Axelson~\cite{svenningsson_combining_2012} propose having a small
 core as the deep embedding. Then, the DSL features are built as a shallow
 layer over the core deep embedding improving its interface. In their approach,
 however, the shallow interface is still modified to hide the IR construction.
