...
To present the translation as a whole we use an example program for calculating $\sum_{i=0}^n i^{exp}$ using the vector \edsl defined in \figref{lst:vector}. \figref{fig:translation-example} contains three versions of the program: \figref{lst:direct-embedding} depicts the direct embedding version, \figref{lst:desugaring} represents the program after type checking (as the translation sees it), and \figref{lst:transformed_program} shows the result of the translation.
...
new VectorDSL with IfOps with MathOps { def main() = {
  $\$$ifThenElse[Int](
    $\$$hole[Int](0, $\$$tpe[Int]) > $\$$lift[Int](0),{ // then
    val v: Rep[Vector[Int]] = $\$$valDef[Vector[Int]](
      $\$$lift(vector.Vector).range(
        $\$$lift[Int](0), $\$$hole[Int]($\$$tpe[Int], 0)))
    v.map[Int](lam[Int, Int](x: Rep[Int] =>
      $\$$lift(math.`package`).pow(x, $\$$hole[Int]($\$$tpe[Int], 1))
